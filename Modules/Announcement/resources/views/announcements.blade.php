...
@section('content')
    <section class="advertisements-page-body">
                ...
                <form class="filters" action="">
                    <div class="row p-2 p-lg-4 mx-0">
                        <div class="col-12 my-1">
                            ...
                            <label for="jobType" class="mt-3">نوع شغل</label>
                            <select class="select2 select2-multiple" name="jobType[]" id="jobType"
                                ...
                                @foreach(\Modules\Resume\Models\Resume::$jobTypes as $jobType)
                                    <option @if(request('jobType') && in_array($jobType,request('jobType'))) selected @endif
                                    value="{{ $jobType }}">{{ __('messages.'.$jobType) }}</option>
                                @endforeach
                            </select>
                            <label for="category" class="mt-3">دسته بندی</label>
        ...
        <div class="advertisements-content">
            ...
            <div class="more-btn">
                <button type="button" id="moreBtn" url="{{ route('announcement.announcementsMore') }}" num="1" class="modern-shadow">
                    موارد بیشتر
                </button>
            </div>
        </div>
