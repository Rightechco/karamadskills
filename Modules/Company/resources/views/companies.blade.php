...
@section('content')
        ...
        @foreach($companies as $company)
            <div class="box_item">
                <a href="{{ route('company.company',$company->slug) }}">
                    <img src="{{ asset($company->logo['indexArray']['medium']) }}" alt="image">
                    <h3>{{ $company->name }}</h3>
                </a>
            </div>
        @endforeach
    ...
    <div class="more-btn">
        <button type="button" id="moreBtn" url="{{ route('company.moreCompanies') }}" num="1" class="modern-shadow">
            موارد بیشتر
        </button>
    </div>
@endsection
