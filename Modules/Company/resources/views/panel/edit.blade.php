                ...
                <div class="col-xl-12 col-lg-12 col-sm-12">
                    <div class="widget-content widget-content-area br-6">
                        <div class="row justify-content-between mx-0 align-items-center">
                            <h4>ویرایش شرکت</h4>
                            <a href="{{ route('panel.company.companies') }}" style="font-size: 35px"><i
                                    class=' fas fa-arrow-alt-circle-left text-info'></i></a>
                        </div>
                        <div class="mb-4 mt-4">
                            <form id="form" action="{{ route('panel.company.companiesUpdate',$company->id) }}" method="post" enctype="multipart/form-data">
                                @csrf
                                ...
                                <div class="border row mt-1 mb-4 p-2 p-lg-4" style="border-radius: .25rem">
                                    ...
                                    <div class="col-12 col-ms-12 col-lg-12 my-1">
                                        ...
                                        <select class="form-control select2" data-live-search="true" id="status"
                                            ...
                                            @foreach(\Modules\Company\Models\Company::$statuses as $status)
                                                <option @if($company->status == $status) selected @endif value="{{ $status }}">{{ __('messages.'.$status) }}</option>
                                            @endforeach
                                        </select>
                                        @error('status')
                                    ...
                                    <div class="col-12 col-ms-6 col-lg-6 my-1">
                                        ...
                                        <input required name="name" type="text" id="name" class="form-control" placeholder="نام" value="{{ old('name',$company->name) }}">
                                        @error('name')
                                    ...
                                    <div class="col-12 col-ms-6 col-lg-6 my-1">
                                        ...
                                        <input name="slug" type="text" class="form-control" id="slug" placeholder="اسلاگ" value="{{ old('slug',$company->slug) }}">
                                        @error('slug')
                                        <small class="form-text text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                    <div class="col-12 col-ms-6 col-lg-6 my-1">
                                        ...
                                        @enderror
                                        @if($company->logo)
                                            <img class="mt-3 w-100" src="{{ asset($company->logo['indexArray'][$company->logo['currentImage']]) }}" alt="">
                                        @endif
                                    </div>
                                    <div class="col-12 col-ms-6 col-lg-6 my-1">
                                        ...
                                        @enderror
                                        @if($company->cover)
                                            <img class="mt-3 w-100" src="{{ asset($company->cover['indexArray'][$company->cover['currentImage']]) }}" alt="">
                                        @endif
                                    </div>
                                    <div class="col-12 col-ms-6 col-lg-6 my-1">
                                        <label for="population">جمعیت شرکت</label>
                                        <input name="population" type="text" id="population" class="form-control" placeholder="مثلا: بین 10 تا 30 نفر" value="{{ old('population',$company->population) }}">
                                        @error('population')
                                        <small class="form-text text-danger">{{ $message }}</small>
                                        @enderror
                                        <label for="website" class="mt-3">آدرس وب سایت</label>
                                        <input name="website" type="text" id="website" class="form-control" placeholder="مثلا: google.com" value="{{ old('website',$company->website) }}">
                                        @error('website')
                                        <small class="form-text text-danger">{{ $message }}</small>
                                        @enderror
                                        <label for="foundation" class="mt-3">زمان تاسیس</label>
                                        <input name="foundation" type="text" id="foundation" class="form-control" placeholder="مثلا: سال 1387" value="{{ old('foundation',$company->foundation) }}">
                                        @error('foundation')
                                        <small class="form-text text-danger">{{ $message }}</small>
                                        @enderror
                                        @can('CompanyPermission')
                                            ...
                                            <select class="form-control select2" id="user" name="user">
                                                ...
                                                @foreach($users as $user)
                                                    @if(isset($user->name))
                                                    <option @if(old('user') == $user->id || $company->user->id == $user->id) selected @endif value="{{ $user->id }}">{{ $user->name }}</option>
                                                    @endif
                                                @endforeach
                                            </select>
                                            @error('user')
                                ...
                                <div class="border row mt-1 mb-4 p-2 p-lg-4" style="border-radius: .25rem">
                                    <div class="col-12 col-md-6 col-lg-6">
                                        ...
                                        <textarea class="col-12" id="expert"
                                                  name="expert">{{ old('expert',$company->expert) }}</textarea>
                                        @error('expert')
                                        <small id="sh-text1" class="form-text text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                    <div class="col-12 col-md-6 col-lg-6">
                                        ...
                                        <textarea class="col-12" id="des" name="des">{{ old('des',$company->des) }}</textarea>
                                        @error('des')
                                        <small id="sh-text1" class="form-text text-danger">{{ $message }}</small>
                                        @enderror
                                    </div>
                                    <div class="col-12 mt-3">
                                        ...
                                        <input type="hidden" name="tags" value="{{ old('tags',$company->tags) }}" id="tags">
...
@section('js')
    ...
    <script>
        ...
        jQuery(document).ready(function() {
            ...
            @if(json_decode($company->location,true)[0])
            var theMarker = L.marker([{{ floatval(json_decode($company->location,true)[0]) }},{{ floatval(json_decode($company->location,true)[1]) }}],
                {icon: preIcon});
