...
@section('title','کارآموزی دانشگاهی')
@section('meta')
...
@section('content')
    ...
    <div class="modal fade" id="rejectModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('panel.intership.rejectIntership') }}" method="post">
                    @csrf
    ...
    <div class="modal fade" id="uniModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('panel.intership.universityAccept') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <input type="hidden" name="id" id="uniRequestId">
                        <label for="file">درج معرفی نامه</label><span class="text-danger">*</span>
    ...
    <div class="modal fade" id="userModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form action="{{ route('panel.intership.userReport') }}" method="post" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        <input type="hidden" name="id" id="userRequestId">
                        <label for="file">درج گزارش پایان کار</label><span class="text-danger">*</span>
                        <input required name="file" type="file" id="file" class="form-control">
                    </div>
                    <div class="modal-footer">
                        <button type="submit" class="btn btn-primary">ارسال</button>
    ...
    <div class="modal fade" id="introductionModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                    <div class="modal-body" id="introductionPic">
...
@section('js')
    ...
    <script>
        $('#companiesTable').DataTable( {
            ...
            processing: true,
            ajax:{
                url: "{{ route('panel.intership.getInterships') }}",
                dataType: "json",
                type: "POST",
        ...
        function setUser(id){
            $('#userRequestId').val(id);
        }

        function setIntroduction(id,e) {
            var url = $(e).attr('lll');
            $('#introductionPic').html('<td>لطفا صبر کنید ...</td>');
            $.ajax({
                type: 'get',
                url: url,
                success: function (list) {
                    $('#introductionPic').html(list);
                }
            });
        }
    </script>
