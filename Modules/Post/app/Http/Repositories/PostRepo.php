...
namespace Modules\Post\Http\Repositories;
    ...
    public static function getPosts($request)
    {
        ...
        if (!empty($post_data)) {
            foreach ($post_data as $key => $post_val) {
                ...
                $postnestedData['detail'] .= '<a href="'.route('panel.post.postEdit',$post_val->id).'" class="btn btn-info m-1" data-toggle="tooltip" data-placement="bottom" title="ویرایش"><i class="mdi mdi-pencil"></i></button>';
                $postnestedData['detail'] .= '<a onClick="sweetConfirm(event)" href="'.route('panel.post.postDelete',$post_val->id).'" class="btn btn-danger m-1" data-toggle="tooltip" data-placement="bottom" title="حذف"><i class="mdi mdi-trash-can"></i></button>';
                $postnestedData['detail'] .= '</td>';
